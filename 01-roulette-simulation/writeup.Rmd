...
套循环？？ 当我们已经拥有上述code后，最后我们需要做的是进行replication, which can be achieved with a for loop or with the replicate command.
瞎写的不知道对不对
看另一个文档
...
For this simulation, we can see that although the gambler has some luck during the game (the probability of making money leaving is 0.5085995), eventually losing out (the average earnings is -5.668305).
...
```{r}
playnumber_before_stopping <- rep(NA, 10)
for(j in seq_along(playnumber_before_stopping)){
  playnumber_before_stopping[j] <- one_series(200,300,1000,100) %>% length()
}
